...
const ProviderDetailsServiceCard = ({ slug, provider, data, compnayName }) => {
  ...
  return (
    <div className={`flex flex-row items-center px-4 py-4 gap-2 card_bg border rounded-lg shadow-sm space-y-4 sm:space-y-0 sm:space-x-4`}>
      ...
      <div className={`flex-1 w-full ${isRTL ? "md:mr-4" : "md:ml-4"}`}>
        ...
        <p className={`text-xs sm:text-sm description_color line-clamp-2 ${isRTL ? 'text-right' : 'text-left'}`}>
              ...
              >
                ...
                {!isMobile ? (
                  ...
                  data?.id && qty[data.id] > 0 ? (
                    ...
                    <button className="w-fit px-3 py-2 text-xs font-medium light_bg_color primary_text_color rounded-md" onClick={(e) => handleAddToCart(e, data)}>
                      {t("addToCart")}
                    </button>
                  )
                )}
